...
function sortList(items) {
    ...
    const result1 = masiv.sort((a, b)=>Number(b.dataset.salary.replace("$", "").replace(",", "")) - Number(a.dataset.salary.replace("$", "").replace(",", "")));
...
function getEmployees(items) {
    ...
    for (const item of items){
        ...
        const salary = Number(item.dataset.salary.replace("$", "").replace(",", ""));
